...
void rising_handler(uint32_t pin)
{
    ...
    printf("last_falling Edge %lu\n", pin_data[pin].last_falling_edge);
    uint32_t current_ticks = get_timer_ticks(TIMER_B);

    printf("current_ticks: %lu\n", current_ticks);
    debug_timer_issue();
...
void falling_handler(uint32_t pin)
{
    ...
    printf("Last falling edge timestamp for pin %u: %lu\n", pin, pin_data[pin].last_falling_edge);
}
