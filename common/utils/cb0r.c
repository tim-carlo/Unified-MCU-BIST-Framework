...
bool cb0r_find(cb0r_t map, cb0r_e type, uint64_t number, uint8_t *bytes, cb0r_t result)
{
  ...
  for (uint32_t i = 0; i < map->length * 2; i += 2)
  {
    ...
    case CB0R_FLOAT:
      ...
      if (number == result->length && memcmp(bytes, result->start + result->header, number) == 0)
        break;
      continue;
    case CB0R_MAP:
    case CB0R_ARRAY:
    case CB0R_TAG:
      ...
      if (number == (uint64_t)(result->end - (result->start + result->header)) && memcmp(bytes, result->start + result->header, number) == 0)
        break;
      continue;
    default:
      continue;
...
uint8_t cb0r_write(uint8_t *out, cb0r_e type, uint64_t number)
{
  ...
  case CB0R_UNDEF:
    ...
    break;
  case CB0R_FLOAT:
  { // incoming number is size of float
    if (number == 2)
      number = 25;
    else if (number == 4)
      number = 26;
